<div id="actionsDropdownContainer">
    ...
    <div id="actionsDropdownContent" data-dropdownId="{{ $query->id }}" class="actionsDropdownContent" style="inset: 0px auto auto 0px; transform: translate3d(105px, -35px, 0px);">
        ...
        <ul class="text-sm text-gray-700 divide-y divide-gray-200">
            ...
            <li class="overflow-hidden">
                <form method="POST" action="{{ route('media.destroy', $query->id) }}" class="w-full inline-flex">
                    @csrf
                    @method('DELETE')
                    <a href="{{ route('media.destroy', $query->id) }}" onclick="event.preventDefault(); this.closest('form').submit();" class="w-full px-4 py-2 hover:bg-gray-100 text-red-500">
                        Delete
                    </a>
                </form>
            </li>
