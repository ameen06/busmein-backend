        ...
        <ul class="text-sm text-gray-700 divide-y divide-gray-200">
            ...
            <li class="overflow-hidden">
                <form method="POST" action="{{ route('destinations.destroy', $query->id) }}" class="w-full inline-flex">
                    @csrf
                    @method('DELETE')
                    <a href="{{ route('destinations.destroy', $query->id) }}" onclick="event.preventDefault(); this.closest('form').submit();" class="w-full px-4 py-2 hover:bg-gray-100 text-red-500">
                        Delete
                    </a>
                </form>
            </li>
